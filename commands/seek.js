  ...
  run: async(client, interaction, message, isSlash, args, manager) => {
    ...
    if (!memberVoiceChannel)
      return respond(
        ...
        message,
        {
          content: "<:uo_music:981587447530029098> You must join a voice channel to use that!",
        },
        isSlash
      );
    if (botVoiceChannel && botVoiceChannel.id !== memberVoiceChannel.id)
      return respond(
        ...
        message,
        {
          content: `<:uo_music:981587447530029098> You must be listening in **${botVoiceChannel.name}** to use that!`,
        },
        isSlash
      );
    ...
    if (!player || !player.queue.current)
      return respond(
        ...
        message,
        {
          content: "<:uo_music:981587447530029098> There must be music playing to use that!",
        },
        isSlash
      );
    let position =
    ...
    if (!position) return respond(
        ...
        message,
        {
          content: "<:uo_music:981587447530029098> - Example `/seek 2m`",
        },
        isSlash
      );
