  ...
  run: (client, interaction, message, isSlash, args, manager) => {
    ...
    if (!memberVoiceChannel)
      return respond(
        ...
        message,
        {
          content: "<:uo_music:981603329211789322> You must join a voice channel to use that!",
        },
        isSlash
      );
    if (botVoiceChannel && botVoiceChannel.id !== memberVoiceChannel.id)
      return respond(
        ...
        message,
        {
          content: `<:uo_music:981603329211789322> You must be listening in **${botVoiceChannel.name}** to use that!`,
        },
        isSlash
      );
    ...
    if (!player || !player.queue.current)
      return respond(
        ...
        message,
        {
          content: "<:uo_music:981603329211789322> There must be music playing to use that!",
        },
        isSlash
      );
    let vol =
    ...
    if (vol < 0 || vol > 150 || isNaN(vol))
      return respond(
        ...
        message,
        {
          content: `<:uo_music:981603329211789322> **Volume must be a valid integer between 0 and 150!**`,
        },
        isSlash
      );
    respond(
      ...
      message,
      {
        content: `<:uo_music:981603329211789322> **Volume changed from \`${player.volume}\` to \`${vol}\` <:uo_music:981603819819520081>**`,
      },
      isSlash
    );
