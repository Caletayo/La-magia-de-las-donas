  ...
  run: async (client, interaction, message, isSlash, args, manager) => {
    ...
    if (!memberVoiceChannel)
      return respond(
        ...
        message,
        {
          content: "<:uo_music:981601635027533825> You must join a voice channel to use that!",
        },
        isSlash
      );
    if (botVoiceChannel && botVoiceChannel.id !== memberVoiceChannel.id)
      return respond(
        ...
        message,
        {
          content: `<:uo_music:981601635027533825> You must be listening in **${botVoiceChannel.name}** to use that!`,
        },
        isSlash
      );
    ...
    if (!player || !player.queue.current)
      return respond(
        ...
        message,
        {
          content: "<:uo_music:981601635027533825> There must be music playing to use that!",
        },
        isSlash
      );
    if (!player.paused)
      return respond(
        ...
        message,
        {
          content: "<:uo_music:981601635027533825> The player is not paused! Use `/pause to` pause!",
        },
        isSlash
      );
    ...
    respond(
      ...
      message,
      {
        content: `<:uo_music:981601635027533825> Resumed **${
          manager.players.get(
            isSlash == true ? interaction.guildId : message.guildId
          )?.queue?.current?.title
        }**`,
